...
// Run the stdio MCP server with `node <tsx-cli> <server.ts>` rather than
// `npx tsx`. On some shared filesystems the `node_modules/.bin/tsx` shim is a
// copied regular file (not the `.mjs` symlink), so node loads it as CommonJS
// and its ESM `import`s throw — the subprocess dies and the agent sees
// "MCP Connection closed". Invoking the .mjs cli directly with the current node
// binary avoids the broken bin shim entirely.
  ...
  if (wantsSubprocess) {
    ...
    return {
      chart_review_state: {
        ...
        args: [TSX_CLI, STDIO_SERVER_PATH],
        env,
      },
    };
  }
