---
field_id: variceal_bleed_date
prompt: Date of the most recent documented variceal hemorrhage, if any
answer_schema:
  type: string
cardinality: one
...
# Criterion: variceal_bleed_date (evidence date)

## Definition
The date of the MOST RECENT documented variceal hemorrhage at ANY time ANYWHERE in the chart — before OR AFTER the index date; there is no time ceiling. Blank when never documented.
