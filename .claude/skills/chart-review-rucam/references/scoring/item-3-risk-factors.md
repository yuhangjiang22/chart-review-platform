...
**Do not answer `item_3_risk_factors` — it is computed.** The scoring thresholds above tell you
what the score *will* be; the platform applies them. What you commit is
`risk_alcohol_or_pregnancy` and `risk_age_ge_55`, and the `item_3_risk_factors` derivation turns
those into the score. `list_criteria` shows the exact leaf fields and their
allowed values. Calling `set_field_assessment` on `item_3_risk_factors`,
