    ...
    <main className="flex-1 min-h-0 flex flex-col overflow-hidden">
      ...
      {mainTab === "notes" && (
        <div className="shrink-0 border-b border-border bg-card px-4 py-2 flex flex-col gap-1.5 max-h-[40%] overflow-y-auto">
           ...
           *  browse uncited notes. Defaults collapsed when the active
           *  criterion has citations (avoid noise during normal review),
           *  defaults expanded otherwise (so a fresh patient/criterion
           *  with nothing cited shows every note immediately). */}
          {otherNotes.length > 0 && (
            <div className="space-y-1">
              <button
              ...
              {(showOtherNotes || (selectedField && citedNotes.length === 0)) && (
                <div className="flex flex-wrap gap-1">
                  {otherNotes.map((n) => (
                    <NoteTab
                      key={n.filename}
...
function NoteTab({ note, active, cited, onClick }: NoteTabProps) {
  return (
    <button
      onClick={onClick}
      className={`px-2.5 py-1 rounded text-[11.5px] whitespace-nowrap border transition-colors ${
        active
      ...
      title={note.filename}
    >
      {note.date ? `${note.date} · ${note.doctype ?? ""}` : note.filename}
    </button>
  );
}
