...
export function App() {
  ...
  // is empty — typical of pilot runs, where the per-agent drafts live in
  // runs/<run>/per_patient/<patient>/agents/ but nothing has materialized
  // them into reviews/<patient>/<task>/review_state.json yet. Without this,
  // PatientDetail shows zero criteria and the reviewer has nothing to
  // override or accept.
  useEffect(() => {
    ...
    (async () => {
      // Walk runs newest-first; the import endpoint refuses if no draft
      // exists for the patient, so we just try the latest one and stop on
      // success.
      const listRes = await authFetch(`/api/runs?task_id=${encodeURIComponent(taskId)}`);
    ...
    >
      {route.page === "tasks" && (
        <TasksIndex
          ...
          onCreateTask={() => setModePickerOpen(true)}
        />
      )}
        ...
        (taskFields.length > 0 ? (
          ...
          <PatientReview
            ...
            onBack={() => {
              ...
              navigate(studioHash(task.task_id, lastStudioSubTabRef.current));
            }}
          />
        ) : (
          <PatientStub
      ...
      {/* Global authoring mode picker — opened from the Tasks index "Create
       *  new task" CTA and the header (when on Tasks). Owns the routing
       *  decision once mode is chosen. */}
      <AuthoringModeDialog
        open={modePickerOpen}
