...
import type { NoteListing, PatientSummary } from "./types.js";

const __filename = fileURLToPath(import.meta.url);
const __dirname = path.dirname(__filename);

// Default to chart-review-platform/corpus/ when run from the platform layout
// (chart-review-platform/app/server/patients.ts → corpus is two parents up).
// Override via env for non-default deployments.
export const PLATFORM_ROOT =
  process.env.CHART_REVIEW_PLATFORM_ROOT ?? path.resolve(__dirname, "../..");
