...
function buildSystemPrompt(task: CompiledTask, entityTypes: string[], yamlBlock: string): string {
  return [
    ...
    `  { "text": string, "anchor": string, "entity_type": string, "concept_name": string, "status": "mapped"|"novel_candidate" }`,
    "  - status='mapped' when concept_name is a real concept in the ontology.",
    "  - status='novel_candidate' with concept_name='' if you can't map it to a known concept.",
    "  - Use the most specific concept available; don't default to the root.",
    "",
...
export async function extractSpansDirect(opts: DirectExtractOpts): Promise<DirectExtractResult> {
  ...
  const systemPrompt = buildSystemPrompt(opts.task, entityTypes, yamls);
  ...
  for (const c of candidates) {
    ...
    const result = await setSpanLabel(session, {
      ...
      status: c.status ?? (c.concept_name ? "mapped" : "novel_candidate"),
    });
    if (result.isError) {
      rejected++;
