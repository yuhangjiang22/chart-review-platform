...
import { loadCompiledTask, listCompiledTasks } from "./lib/tasks.js";
...
// Default task when a request/subscribe omits one: env override, else the
// first discovered task (no task name baked in — works for any deployment).
const DEFAULT_TASK_ID =
  process.env.CHART_REVIEW_TASK_ID ?? listCompiledTasks()[0]?.task_id ?? "";
