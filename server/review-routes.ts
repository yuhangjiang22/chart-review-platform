  ...
  // derived (every-note-validated → patient is done).
  {
    ...
    handler: async (body, req, p, query) => {
      ...
      return withReviewsRoot(sessionReviewsRoot(sid), async () => {
        ...
        // Maintain the top-level review_status from note-validation progress so
        // the patient shows as validated OUTSIDE this pane (SessionSidebar
        // oracle_done, GET /api/patients, performance, export) — all of which
        // gate on review_status === "reviewer_validated". Not locked here (the
        // guard above already 409'd on locked).
        const derived = deriveNerReviewStatus(state);
