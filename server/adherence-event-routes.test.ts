...
describe("POST .../adherence/add-event", () => {
  const addEventRoute = findRoute("/api/reviews/:patientId/:taskId/adherence/add-event");
  const ADD_PID = "p_addevent";

  function callAdd(body: unknown, patientId = ADD_PID): Promise<unknown> {
    return addEventRoute.handler(
      body,
      {} as unknown as IncomingMessage,
      { patientId, taskId: TASK_ID },
      query(),
  ...
  beforeAll(() => {
    // Own rule file, additive to rubricRoot — two declared anchors so the
    // anchor_type validation has something to reject against.
    fs.writeFileSync(
      path.join(rubricRoot, "references", "rules", "rules-add-event.yaml"),
      [
        "rules:",
        "  - rule_id: R-Supplementable",
        "    description: anchored rule a reviewer may supplement from a note",
        "    verdict_if: StepMatch == \"matches\"",
  ...
  it("adds a note-origin event the structured enumeration missed", async () => {
    ...
    const state = await readStateFor(ADD_PID);
    const ev = state.rule_events!.find((e) => e.event_id === res.event_id)!;
    expect(ev.rule_id).toBe("R-Supplementable");
    expect(ev.anchor).toMatchObject({ type: "bursts", date: "2025-03-04", origin: "note", ref: "note:note_17" });
    expect(ev.source).toBe("reviewer");
    // Created unanswered: the annotator answers it through event-verdict, so
    // there is nothing here for the engine to derive a verdict from yet.
    expect(ev.answers ?? []).toHaveLength(0);
    expect(ev.verdict).toBeUndefined();
  });

  it("mints the same event_id the agent's supplement would, so one episode found by both sides is ONE key", async () => {
    // Per-event IAA matches on `${patient_id}|${event_id}`, so the reviewer
    // and agent supplement paths must agree on the id. Both build it as
    // `${rule_id}@${date}@note` — note_id deliberately absent, so the sides
    // still match when they cite different notes for the same visit. The
    // counterpart assertion lives in mcp-core-adherence's new_event branch.
    const res = (await callAdd({
      rule_id: "R-Supplementable", date: "2025-08-09", note_id: "note_30",
    })) as { event_id: string };
    expect(res.event_id).toBe("R-Supplementable@2025-08-09@note");
    expect(res.event_id).not.toContain("note_30");
  });

  it("defaults anchor_type to the rule's first declared anchor", async () => {
    const res = (await callAdd({
      rule_id: "R-Supplementable", date: "2025-04-05", note_id: "note_18",
    })) as { event_id: string };
    const state = await readStateFor(ADD_PID);
    const ev = state.rule_events!.find((e) => e.event_id === res.event_id)!;
    expect(ev.anchor.type).toBe("visits");
  });

  it("rejects an anchor_type the rule does not declare", async () => {
    // It surfaces to expressions as `_anchor_type`; a free-text value fails
    // every `_anchor_type == "..."` comparison silently.
    await expect(callAdd({
      rule_id: "R-Supplementable", anchor_type: "ocs_burst", date: "2025-05-06", note_id: "n1",
    })).rejects.toMatchObject({ status: 400 });
  });

  it("rejects a window-scoped rule — it already has a stub covering the period", async () => {
    await expect(callAdd({
      rule_id: "R-Other", date: "2025-05-06", note_id: "n1",
    })).rejects.toMatchObject({ status: 400 });
  });

  it("rejects an unknown rule_id", async () => {
    await expect(callAdd({
      rule_id: "R-Nope", date: "2025-05-06", note_id: "n1",
    })).rejects.toMatchObject({ status: 404 });
  });

  it("requires a well-formed date and a citing note", async () => {
    await expect(callAdd({ rule_id: "R-Supplementable", date: "03/04/2025", note_id: "n1" }))
      .rejects.toMatchObject({ status: 400 });
    await expect(callAdd({ rule_id: "R-Supplementable", date: "2025-06-07" }))
      .rejects.toMatchObject({ status: 400 });
  });

  it("409s rather than duplicating when the same episode is added twice", async () => {
    const body = { rule_id: "R-Supplementable", date: "2025-07-08", note_id: "note_20" };
    await callAdd(body);
    await expect(callAdd({ ...body, note_id: "note_21" }))
      .rejects.toMatchObject({ status: 409 });
    const state = await readStateFor(ADD_PID);
    expect(state.rule_events!.filter((e) => e.anchor.date === "2025-07-08")).toHaveLength(1);
  });
});
