  ...
  // ── /api/pilots/:taskId/:iterId/* ────────────────────────────────
  {
    ...
    handler: async (_b, _r, p) => {
      ...
      const patient_status = patientIds.map((pid) => {
        ...
        // Agent still in flight (pending/running) — distinct from "finished
        // with no draft". The UI shows "agent running…" rather than inviting
        // premature hand-validation while the run is still producing drafts.
        const agentRunning = perPatient?.state === "pending" || perPatient?.state === "running";
        ...
        if (reviewPath && fs.existsSync(reviewPath)) {
          try {
            ...
            // Validation is a single ground truth (validated once, e.g. run 1);
            // it doesn't change per run. Performance compares each run to it.
            validated =
...
export const pilotWriteRoutes: RouteEntry[] = [
  ...
  // POST /api/pilots/:taskId/:iterId/judge — start judge batch (async)
  {
    ...
    handler: async (_body, req, p) => {
      ...
      // (the schema carries a `task_kind` discriminator). The shared
      // lock above prevents double-starts for either kind.
