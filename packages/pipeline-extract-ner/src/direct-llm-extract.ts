...
export interface DirectExtractOpts {
  ...
  reviewsRoot: string;
  sessionId: string;
  /** LLM endpoint base URL (e.g. the OpenRouter base, or an Azure
   *  Responses endpoint). The transport shape is chosen by `mode`. */
  baseUrl: string;
  apiKey: string;
  model?: string;
  /** Transport selector. concur runs on OpenRouter
   *  (`/chat/completions`); `"azure-responses"` retains v2's Azure shape.
...
export async function extractSpansDirect(opts: DirectExtractOpts): Promise<DirectExtractResult> {
  ...
  // Identification pass — one LLM call per note (no streaming, no agent
  // loop). Transport is chosen by `mode` (OpenRouter by default; Azure
  // Responses retained behind `mode: "azure-responses"`). The prompt
  // building above and the JSON parsing below are unchanged — only the
  // transport moved into `callLlm`.
  const mode: LlmMode = opts.mode ?? "openrouter";
  let outputText: string;
  let usage: DirectExtractResult["usage"];
  try {
    const res = await callLlm(
      { baseUrl: opts.baseUrl, apiKey: opts.apiKey, model: opts.model ?? "gpt-5.2", mode },
      systemPrompt,
      userPrompt,
      4096,
    );
