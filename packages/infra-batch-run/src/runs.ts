  ...
  await withReviewsRoot(scratchRoot, async () => {
    ...
    if (isAdherenceTask) {
      ...
      const adherenceUserPrompt = [
        ...
        "Step 4: For each question from step 1 that is NOT marked",
        "        `event_scoped`, call `set_question_answer({",
        "        question_id, answer, confidence, evidence, reasoning})` exactly",
        "        once. An `event_scoped` question describes ONE event, not the",
        "        period, so it has no single period answer — commit it per event",
        "        in Step 4b instead; set_question_answer rejects it here. Use structured data when the question's retrieval_hints",
        ...
        "        EVIDENCE SOURCE — when your answer is DETERMINED from a structured",
        "        OMOP row (drugs, encounters, measurements, conditions,",
        "        observations), cite THAT row: evidence:[{source:'omop', table,",
        "        row_id}] — do NOT cite a note for a structured-data answer. OMOP",
        "        evidence needs table+row_id and NO quote (only note quotes are",
        "        faithfulness-checked). Controller-prescribed, SABA-overuse,",
        "        exacerbation-count, ACT score, spirometry-date, and the",
        ...
        "  - list_criteria / read_criterion(field_id)  (reference docs a question's",
        "    retrieval_hints point at — e.g. the verbatim step-therapy ladders and",
        "    ICS dose tables. When a hint says to read one FIRST, do so before",
        "    answering that question; judge from the file, not trained memory.)",
        "",
