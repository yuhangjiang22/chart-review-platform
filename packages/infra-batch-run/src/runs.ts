...
async function runOneAgent(
  ...
  const phenotypePrompt = [
    ...
    "- For ANY chart content (notes, structured tables): use the chart_review_state",
    "  MCP tools — `list_notes`, `read_notes` / `read_note`, `list_structured_data`,",
    "  `read_structured_data`. Do NOT `cat`/`sed`/`ls`/`rg` the filesystem to read",
    "  patient files.",
  ...
  await withReviewsRoot(scratchRoot, async () => {
    ...
    if (isAdherenceTask) {
      ...
      const adherenceUserPrompt = [
        ...
        "",
        "Step 1: call `list_questions` (no args) to see every question_id and",
        "        its tier / answer_schema.",
        "Step 2: call `list_notes` (no args) to see filenames, then",
        "        `read_notes({filenames: [...all of them...]})` in ONE batch call.",
        "Step 3: For each question from step 1, call `set_question_answer({",
        "        question_id, answer, confidence, evidence, reasoning})` exactly",
        "        once. The platform coerces `answer` to the question's schema;",
        "        pass `null` when the chart doesn't support an answer (better",
        "        than guessing).",
        "Step 4: After every question has been answered, call",
        "        `set_review_status({status: 'complete'})`. The platform runs the",
        "        rule engine + LLM judge afterwards — you DO NOT compute rule",
        "        verdicts yourself.",
        "",
        "Active tools (use ONLY these — listed in the order you'll call them):",
        "  - list_questions         (catalog of questions, call once)",
        "  - list_notes             (catalog of notes, call once)",
        "  - read_notes             (bulk note reader, call once with all filenames)",
