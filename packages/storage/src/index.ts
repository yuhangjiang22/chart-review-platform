...
export const pathFor = {
  ...
  reviewState(sessionId: string, patientId: string, taskId: string): string {
    // Honor CHART_REVIEW_REVIEWS_ROOT (the env var the rest of the platform
    // reads) so review data can live outside the vendored platform dir.
    const root =
      process.env.CHART_REVIEW_REVIEWS_ROOT ?? path.join(PLATFORM_ROOT, "var", "reviews");
    return path.join(root, sessionId, patientId, taskId, "review_state.json");
  },
  ...
  agentDraft(runId: string, patientId: string, agentId: string): string {
    ...
    return path.join(root, runId, "per_patient", patientId, "agents", `${agentId}.json`);
  },
