...
export const defaultPhenotypeProfile: ExtractorProfile = {
  ...
  buildPrompt(ctx) {
    ...
    return [
      ...
      "",
      "Procedure:",
      "1. Call `list_criteria` then `read_criteria` to get each field's allowed",
      "   enum values and extraction guidance — follow the criterion's mappings",
      "   and `no_info` rules exactly.",
      "2. Find evidence in the patient's notes (under your cwd). On a long chart,",
      "   use `search_notes` for high-signal terms ('final diagnosis', 'metastatic',",
      "   'stage IV', 'M1', 'recurrence') to locate spans, then `read_note`/",
      "   `read_notes` the candidates. Prefer pathology 'FINAL DIAGNOSIS' first.",
      "3. Commit ONE answer per leaf criterion via `set_field_assessment`",
      "   (answer, confidence, evidence, rationale). The answer MUST be an enum",
      "   value; use the field's `no_info` value when the notes don't document it.",
      "   Cite the SMALLEST verbatim span (one sentence/phrase, well under ~300",
      "   chars) — call `find_quote_offsets` BEFORE citing so faithfulness passes.",
      "   NEVER cite a whole note.",
      "4. Commit EVERY leaf criterion before finishing. Do NOT call",
      "   `set_review_status` — it is unnecessary here and HARD-FAILS the run if any",
      "   field is still uncommitted. When all criteria are committed, emit a",
  ...
  buildExtraSystemPrompt() {
    return (
      ...
      "evidence available. " +
      "Extract ONLY what is affirmatively documented for THIS patient — never " +
      "infer, assume, or guess. Exclude negated findings (\"no evidence of…\", " +
      "\"rule out…\") and family history. Use the field's `no_info` value when the " +
      "notes do not document it; do not fall back to a plausible-sounding answer. " +
      "Apply the criterion's source-document priority and cite the smallest " +
      "verbatim span as evidence."
    );
  },
