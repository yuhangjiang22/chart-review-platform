...
import { PhaseAdherenceAuthor } from "./PhaseAdherenceAuthor";
...
const REGISTRY: Record<TaskKind, TaskKindUiBundle> = {
  ...
  adherence: {
    kind: "adherence",
    authorPane: PhaseAdherenceAuthor as AuthorPaneComponent,
    reviewerPane: AdherenceReview as ReviewerPaneComponent,
    unitLabel: { singular: "question", plural: "questions" },
  },
};

/**
 * Resolve a task-kind to its UI bundle. Accepts the raw meta.yaml
 * `task_type` (e.g. "phenotype_validation", "ner", "adherence") so
 * callers don't need to normalize first; unknown values default to
...
export function taskKindUi(rawTaskType: string | undefined): TaskKindUiBundle {
  if (rawTaskType === "ner") return REGISTRY.ner;
  if (rawTaskType === "adherence") return REGISTRY.adherence;
  return REGISTRY.phenotype;
}

/** Normalize the raw meta.yaml `task_type` to a TaskKind discriminator
 *  used by phase-pane props (PhaseValidate/Decide/Lock). Mirrors the
 *  server-side `taskKindFromTaskType` so the client doesn't drift. */
export function taskKindFromTaskType(rawTaskType: string | undefined): TaskKind {
  if (rawTaskType === "ner") return "ner";
  if (rawTaskType === "adherence") return "adherence";
  return "phenotype";
}
