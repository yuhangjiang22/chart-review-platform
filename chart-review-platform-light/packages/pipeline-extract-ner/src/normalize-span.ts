...
async function callAzure(opts: NormalizeSpanOpts, system: string, user: string): Promise<ProviderCallResult> {
  const url = `${opts.azureBaseUrl ?? ""}/responses`;
  try {
    const r = await fetch(url, {
      method: "POST",
      headers: { "api-key": opts.azureApiKey ?? "", "Content-Type": "application/json" },
      body: JSON.stringify({
    ...
    if (!r.ok) {
      ...
      return { text: "", error: `HTTP ${r.status}: ${body.slice(0, 200)}` };
    }
    ...
    if (body.error) return { text: "", error: body.error.message ?? "azure error" };
    let outputText = "";
    ...
    return { text: outputText, usage: body.usage };
...
export async function normalizeSpanWithLLM(opts: NormalizeSpanOpts): Promise<NormalizeSpanResult> {
  ...
  const res = (opts.provider ?? "codex") === "claude"
    ...
    : await callAzure(opts, system, user);
  if (res.error) {
