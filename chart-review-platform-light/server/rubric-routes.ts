...
/** Validate an identifier for use in a file path. Rejects anything that
 *  could traverse outside the criteria directory. */
...
function atomicWriteText(filePath: string, content: string): void {
  ...
  const tmp = path.join(os.tmpdir(), `rubric-${Date.now()}-${Math.random().toString(36).slice(2)}.tmp`);
 ...
 * Returns { frontmatter, definition, extraction_guidance, examples }
 * plus any extra heading sections keyed by heading text.
 */
function parseCriterionMd(raw: string): {
  ...
  // Extract named ## sections from the body.
...
/**
 * Reconstruct a criterion markdown file from its parts.
 * Frontmatter is written in the canonical order:
 *   field_id, prompt, answer_schema: { enum }, cardinality, group
 * Body sections: # Criterion, ## Definition, ## Extraction guidance, ## Examples
 */
function buildCriterionMd(params: {
  field_id: string;
  prompt: string;
  enumValues: string[];
  cardinality?: string;
  ...
  const fm: Record<string, unknown> = {
    ...
    answer_schema: { enum: params.enumValues },
    cardinality: params.cardinality ?? "one",
    group: params.group ?? "",
  };
  // Build YAML frontmatter manually to preserve order (yaml.stringify sorts keys
  // by default but we can pass a stable representation).
  const yamlLines = [
    `field_id: ${fm.field_id}`,
    `prompt: ${stringifyYaml(fm.prompt as string).trimEnd()}`,
    `answer_schema:`,
    `  enum:`,
