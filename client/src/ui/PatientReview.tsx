...
export interface PatientReviewProps {
  ...
  onBack?: () => void;
  /** Open another patient directly (used by "Mark validated & next" /
   *  "Next patient" to jump to the next cohort patient without a grid round-
   *  trip). When omitted, those actions fall back to onBack. */
...
export function PatientReview(p: PatientReviewProps) {
  ...
  // The socket-provided reviewState lags a patient switch: for a moment after
  // navigating, p.reviewState still belongs to the PREVIOUS patient. Using it
  // would seed the criterion form with the prior patient's answer (which the
  // reviewer then submits unchanged → wrong label). Only trust reviewState
  // once its patient_id matches the patient we're actually viewing.
  const reviewFresh = !!p.reviewState && p.reviewState.patient_id === p.patientId;
  ...
  useEffect(() => {
    let cancelled = false;
    (async () => {
      // Scope to THIS session's own runs so VALIDATE shows the session's
      // latest results, never another session's run that happens to cover
      // the same patient.
      const runsQs =
        `?task_id=${encodeURIComponent(p.taskId)}` +
        (p.activeSessionId ? `&session_id=${encodeURIComponent(p.activeSessionId)}` : "");
      ...
      for (const run of runs) {
        ...
        // Ignore failure markers (<agent>.error → 0 field_assessments). An
        // agent that errored in the newest run must NOT stop the walk on an
        // empty draft — otherwise older runs with a real draft are never
        // reached and the card shows "no agent output".
        const drafts = (body.drafts ?? []).filter(
          (d) => !d.agent_id.endsWith(".error") && (d.field_assessments?.length ?? 0) > 0,
        );
  ...
  // Header-level validate + advance. Duplicated from the footer so the key
  // action is in the always-visible top bar (the footer can sit below the
  // fold). Posts the same /validate endpoint, then returns to the grid.
  // Scroll the criterion pane back to the top whenever the active criterion
  // changes, so "Submit & next criterion" lands the reviewer at the top of the
  // next one instead of mid-scroll.
  const criterionScrollRef = useRef<HTMLDivElement>(null);
  useEffect(() => {
    criterionScrollRef.current?.scrollTo({ top: 0 });
  }, [selectedFieldId]);

  const [headerValidating, setHeaderValidating] = useState(false);
  // The next patient in this session's cohort (or null if last / unknown).
  async function nextCohortPatient(): Promise<string | null> {
    if (!p.activeSessionId) return null;
    try {
      const r = await authFetch(`/api/sessions/${p.taskId}`);
      const d = await r.json();
      const sess = (d.sessions ?? [])
        .map((x: { session?: { session_id?: string; cohort?: { patient_ids?: string[] } } }) => x.session)
        .find((s) => s?.session_id === p.activeSessionId);
      const ids: string[] = sess?.cohort?.patient_ids ?? [];
      const i = ids.indexOf(p.patientId);
  ...
  async function goToNextPatient() {
    const next = await nextCohortPatient();
    if (next && p.onOpenPatient) p.onOpenPatient(next);
    else p.onBack?.();   // last patient → back to the grid
  }
  async function validateAndNext() {
    if (terminal !== leaves.length || headerValidating) return;
    setHeaderValidating(true);
    try {
      const qs = p.activeSessionId ? `?session_id=${encodeURIComponent(p.activeSessionId)}` : "";
      const r = await authFetch(`/api/reviews/${p.patientId}/${p.taskId}/validate${qs}`, { method: "POST" });
      const body = await r.json();
      if (!body.ok) { alert(`Cannot validate yet:\n${JSON.stringify(body.gate_results, null, 2)}`); return; }
      if (body.state) p.onStateChanged(body.state);
      await goToNextPatient();   // jump straight to the next patient
    } finally {
    ...
    <div className="flex h-full flex-col min-h-0 bg-paper">
      ...
      <div className="flex flex-1 min-h-0 overflow-hidden">
        <main className="flex-1 min-w-0 flex flex-col">
          ...
          />
          <div ref={criterionScrollRef} className="flex-1 min-h-0 overflow-y-auto px-6 py-5">
            <div className="mx-auto max-w-[820px]">
              {selected ? (
                <>
                  ...
                  <CriterionCard
                    key={`${p.patientId}:${selected.field.id}:${reviewFresh ? "fresh" : "stale"}`}
                    ...
                    onSubmit={async ({ field_id, answer, evidence, rationale, comment }) => {
                      ...
                      try {
                        ...
                        if (!res.ok || data?.ok === false) {
                          // Surface the failure (e.g. faithfulness gate rejecting
                          // an evidence quote) instead of silently leaving the
                          // field uncommitted — that looked like "Submit does nothing".
                          alert(
                            "Could not save this answer:\n" +
                            (data?.message || data?.error || JSON.stringify(data?.gate_results ?? data) || `HTTP ${res.status}`) +
                            "\n\nTip: cite a verbatim quote from the note (use 'Copy from Agent' or select text in the Notes pane), then Submit.",
                          );
                        } else if (data?.state) {
                          p.onStateChanged(data.state);
                          committed = true;
                        }
                      } catch { /* non-JSON / error already surfaced */ }
                      // Only advance if the field actually committed — otherwise
                      // stay so the reviewer can fix it.
                      if (committed) jumpToNextPending();
                    }}
      ...
      <ReviewFooter
        ...
        onValidateAndNext={validateAndNext}
...
function ReviewFooter({
  ...
  activeSessionId?: string | null;
  onStateChanged: (state: ReviewState) => void;
  /** Validate this patient, then jump straight to the next cohort patient. */
  onValidateAndNext: () => void;
  /** Go to the next cohort patient without validating (already-validated). */
  onNextPatient: () => void;
  validating: boolean;
}) {
  ...
  return (
    <footer className="flex h-11 shrink-0 items-center gap-3 border-t border-border bg-card px-5">
      <span className={`text-[12px] ${terminal === leaves.length && !isValidated ? "font-medium text-foreground" : "text-muted-foreground"}`}>
        {isValidated
          ? "Validated ✓"
          : terminal === leaves.length
          ? "All criteria reviewed — click Mark validated to finish this patient →"
          : `${leaves.length - terminal} criteria still need a decision.`}
      </span>
      <span className="ml-auto flex items-center gap-2">
        {!isValidated ? (
          <Button
            variant={terminal === leaves.length ? "default" : "secondary"}
            size="sm"
            onClick={onValidateAndNext}
            disabled={validating || terminal !== leaves.length}
            title={
              terminal !== leaves.length
                ? `${leaves.length - terminal} criteria still pending`
                : "Mark this record validated, then go to the next patient"
            }
          >
          ...
          <>
            ...
            <Button variant="secondary" size="sm" onClick={onNextPatient} title="Go to the next patient in the cohort">
              Next patient →
            </Button>
          </>
        )}
