...
export function PhaseTry({
  ...
  // Poll while the run is in flight — backstop only. The WS subscription
  // below is the primary, near-instant update path; this just covers a
  // dropped socket. (Was 4s and the *only* path, which is why the progress
  // counter/badge lagged seconds behind per-patient drafts that had already
  // landed.)
  useEffect(() => {
  ...
  // Real-time: the server broadcasts `agent_run_update` on EVERY per-patient
  // state transition (running → complete/error) via mutate()→onStatus. Refresh
  // the moment one arrives so the "N/M patients drafted" counter and the
  // RUNNING badge track reality instead of waiting up to 4s for the next poll.
  // We treat the broadcast as a refresh trigger (re-fetch the iter) rather than
  // trusting its payload, so the panel stays consistent with the REST view.
  useEffect(() => {
    ...
    try {
      ...
      return; // WS unavailable → the poll backstop above still updates.
    }
    ws.onmessage = (ev) => {
      let msg: { type?: string } = {};
      try { msg = JSON.parse(ev.data as string); } catch { return; }
      if (msg.type === "agent_run_update") loadActiveIter();
    };
    return () => ws?.close();
  }, [activeSessionId, loadActiveIter]);
