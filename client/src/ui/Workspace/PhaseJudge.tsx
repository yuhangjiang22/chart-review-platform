...
// disagreements + low-confidence cells + type-drift cells, writes
// <task>/pilots/<iter>/judge_analyses.json, and PatientReview reads that
// file to surface an advisory panel per criterion in VALIDATE.
//
// Phenotype-only (concur). The judge runs on the run's provider
// (deepagents/Azure) via the provider-agnostic runAgent — see
// server/lib/judge.ts. This phase is OPTIONAL: reviewers who want to
// validate without LLM pre-screening simply navigate past it.
...
interface PhaseJudgeProps {
  ...
  /** Kept for API compatibility with the Workspace caller — phenotype only. */
  taskKind?: "phenotype";
}
