...
/** Parsed adherence proposal shape — written by chart-review-adherence-improve. */
interface AdherenceProposalDoc {
  proposal_id?: string;
  target_file?: string;
  change_kind?: string;
  question_id?: string;
...
interface ImprovementProposalsPanelProps {
  taskId: string;
  patientIds: string[];
  /** Bumped by the parent after a successful improvement run so the
   *  panel re-fetches the proposals list without a manual reload. */
  refreshKey?: number;
}
...
/** Human-readable label for the adherence proposal change_kind. */
function adherenceChangeKindLabel(kind: string | undefined): string {
  switch (kind) {
    case "edit_retrieval_hints": return "sharpens retrieval hints";
    case "edit_question_text":   return "rewrites question text";
    case "edit_answer_schema":   return "tightens answer schema";
    case "add_depends_on":       return "adds depends_on guard";
    case "edit_rule":            return "edits rule logic";
    default: return (kind ?? "").replace(/_/g, " ");
  }
}

/** Truncate a long string for compact answer-diff display. */
...
export function ImprovementProposalsPanel({
  ...
  /** Parsed proposal doc per proposal id. NerProposalDoc and
   *  AdherenceProposalDoc share enough fields that we keep the same map
   *  and discriminate at render time by checking for `entity_type`
   *  (NER) vs `question_id|rule_id` (adherence). Phenotype proposals
   *  match neither and fall through to the raw-YAML listing. */
  const [parsed, setParsed] = useState<Record<string, NerProposalDoc | AdherenceProposalDoc | "yaml-error">>({});
    ...
    for (const p of proposals) {
      ...
      authFetch(
        ...
        .then((text) => {
          ...
          try {
            const doc = parseYaml(text) as NerProposalDoc & AdherenceProposalDoc;
        ...
        <div className="space-y-3">
          ...
          <ul className="space-y-3">
            {proposals.map((p, i) => {
              ...
              const doc = parsed[id];
              const isNer = doc && doc !== "yaml-error"
                && typeof (doc as NerProposalDoc).entity_type === "string";
              const isAdherence = !isNer && doc && doc !== "yaml-error"
                && (typeof (doc as AdherenceProposalDoc).question_id === "string"
                  || typeof (doc as AdherenceProposalDoc).rule_id === "string");
              ...
              return (
                ...
                >
                  ...
                  {/* Friendly adherence render. Mirrors the NER card but
                   *  with answer-diff evidence instead of span examples,
                   *  and the question-id badge in place of entity_type. */}
                  {isAdherence && doc !== "yaml-error" && (() => {
                    const ad = doc as AdherenceProposalDoc;
                    const targetId = ad.question_id ?? ad.rule_id ?? "";
                    const targetKind = ad.question_id ? "question" : "rule";
                    return (
                      <div className="p-4 space-y-3">
                        {/* Header: question_id badge + change_kind verb */}
                        <div className="flex items-center gap-2 flex-wrap">
                          <Sparkles size={14} className="text-[hsl(var(--ochre))] shrink-0" />
                          <span className="text-[10.5px] uppercase tracking-[0.14em] font-medium text-[hsl(var(--ochre))]">
                            {targetId}
                          ...
                          <span className="text-[10.5px] text-muted-foreground">·</span>
                          <span className="text-[10.5px] uppercase tracking-[0.14em] text-muted-foreground">
                            {adherenceChangeKindLabel(ad.change_kind)}
                          </span>
                        </div>
                        ...
                        {/* Evidence: per-example agent vs reviewer diff */}
                        {ad.evidence && (
                          <div className="text-[11.5px] text-muted-foreground space-y-1">
                            <div>
                              <strong className="text-foreground">Evidence:</strong>{" "}
                              {ad.evidence.patient_ids?.length ?? 0} patient
                              {(ad.evidence.patient_ids?.length ?? 0) === 1 ? "" : "s"}
                              {ad.evidence.examples && ad.evidence.examples.length > 0 && (
                                <>, {ad.evidence.examples.length} disagreement
                                  {ad.evidence.examples.length === 1 ? "" : "s"}</>
                              )}
                            </div>
                            {ad.evidence.examples && ad.evidence.examples.length > 0 && (
                              <ul className="list-disc pl-5 mt-1 space-y-1">
                                {ad.evidence.examples.slice(0, 3).map((ex, j) => (
                                  <li key={j} className="space-y-0.5">
                                    <div>
                                      <code className="text-[11px]">{ex.question_id ?? ex.rule_id}</code>:{" "}
                                      <span className="text-[hsl(var(--oxblood))] font-mono">
                                        agent {fmtAnswer(ex.agent_answer ?? ex.agent_verdict)}
                                      </span>
                                      {" → "}
                                      <span className="text-[hsl(var(--sage))] font-mono">
                                        reviewer {fmtAnswer(ex.reviewer_answer ?? ex.reviewer_verdict)}
                                      </span>
                                    </div>
                                    {ex.agent_rationale && (
                                      <div className="text-[10.5px] italic pl-1">
                                        agent: "{ex.agent_rationale.slice(0, 120)}{ex.agent_rationale.length > 120 ? "…" : ""}"
                                      </div>
                                    )}
                                    {ex.reviewer_rationale && (
                                      <div className="text-[10.5px] italic pl-1">
                                        reviewer: "{ex.reviewer_rationale.slice(0, 120)}{ex.reviewer_rationale.length > 120 ? "…" : ""}"
                                      </div>
                                    )}
                                  </li>
                                ))}
                                {ad.evidence.examples.length > 3 && (
                                  <li className="italic">
                                    + {ad.evidence.examples.length - 3} more
                                  </li>
                                )}
                              </ul>
                            )}
                        ...
                        <div className="text-[11.5px] text-muted-foreground">
                          <strong className="text-foreground">If accepted:</strong>{" "}
                          patches <code>{ad.target_file ?? "(target_file missing)"}</code>{" "}
                          → {targetKind} <code>{targetId}</code>
                        </div>

                        {/* Action buttons. Accept calls the same /apply
                         *  endpoint as NER — server dispatches by task_kind
                         *  and patches the matching question / rule entry
                         *  in target_file. */}
                        <div className="flex items-center gap-2 pt-1">
                          <Button
                            size="sm"
                  ...
                  {!isNer && !isAdherence && (
                    <div className="p-4 space-y-2 text-[12.5px]">
                      ...
                      <div className="text-[10.5px] text-muted-foreground">
                        {!doc && "Loading body…"}
                        {doc === "yaml-error" && "Couldn't parse this proposal — open the YAML below."}
                        {doc && doc !== "yaml-error" && "Phenotype rule proposal — Accept is handled in the Lock → Drain rule queue."}
                      </div>
                      <Button
                        size="sm"
                        variant="ghost"
